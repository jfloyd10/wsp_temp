...
def get_pnl_filter_options() -> dict:
    ...
    try:
        ...
        return {
            'entities': [], 'entity_classes': [], 'categories': [],
            'years': [], 'covered_uncovered': [],
        }


def get_pnl_income_statement(filters: dict) -> list[dict]:
    """Return P&L data grouped for a traditional income statement view.
...
<<<<<<< HEAD
            ...
            ORDER BY
                CASE category
                    ...
                    ELSE 5
                END,
                type, subtype, line_item
        """, params).fetchall()
        columns = ['category', 'type', 'subtype', 'line_item', 'total_amount', 'row_count']
        conn.close()
        return [dict(zip(columns, row)) for row in rows]
    except Exception:
...
def get_pnl_monthly_trend(filters: dict) -> list[dict]:
    """Return monthly P&L totals by category for trend charts."""
    try:
        conn = get_connection()
        conditions = []
        params = []
...
def get_pnl_yoy_comparison(filters: dict) -> list[dict]:
    """Return year-over-year P&L comparison by category."""
    try:
        conn = get_connection()
        conditions = []
        params = []
...
def get_pnl_entity_comparison(filters: dict) -> list[dict]:
    """Return P&L totals by entity for comparison charts."""
    try:
        conn = get_connection()
        conditions = []
        params = []
        ...
        if filters.get('covered_or_uncovered'):
            conditions.append("covered_or_uncovered = ?")
            params.append(filters['covered_or_uncovered'])
